...
exports.compilePython = function (envData , code , fn){
	...
	fs.writeFile( path  +  filename +'.py' , code  , function(err ){			
		...
		if(!err)
		{
			var command = 'docker cp ./temp/'+filename+'.py  compiler:/main.py &&docker exec -i compiler python3 main.py';
	...
	fs.writeFile( path  +  filename +'.py' , code  , function(err ){			
		...
		if(!err)
		{
			fs.writeFile(path + filename + 'input.txt' , input , function(err){
				...
				if(!err)
				{
					var command = 'docker cp ./temp/'+filename+'.py compiler:/main.py && docker cp ./temp/'+filename+'input.txt  compiler:/input.txt && docker exec -i compiler python3 main.py < ./temp/'+filename+'input.txt';
					exec( command , function ( error , stdout , stderr ){
						if(error)
						{
							if(error.toString().indexOf('Error: stdout maxBuffer exceeded.') != -1)
